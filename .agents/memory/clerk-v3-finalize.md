...
## Rule
Never pass a `navigate` callback to `signIn.finalize()` or `signUp.finalize()` in Expo web. Call `finalize()` with no args, then navigate manually with `router.replace("/")`.
