...
// ── Mock nearby hikes ─────────────────────────────────────────────────────────
// TODO: Replace with real trail API integration (e.g. OpenStreetMap Overpass, OS Maps, AllTrails API)
// When available: fetch from /api/hikes-near-me?lat=...&lng=...&radius=...
...
const MOCK_HIKES: MockHike[] = [
  ...
  { name: "Blencathra via Sharp Edge", distance: 9.5, duration: 280, elevationGain: 670, difficulty: "Hard", trainingValue: "strong", region: "Lake District" },
...
const DIFF_COLOR = { Easy: T.green, Moderate: T.blue, Hard: T.orange };

function HikeCard({ hike, onLog }: { hike: MockHike; onLog: (name: string) => void }) {
  return (
    <View style={hc.card}>
      <View style={hc.top}>
      ...
      <View style={hc.stats}>
        ...
        <View style={hc.stat}>
          ...
          <Text style={hc.statText}>{Math.floor(hike.duration / 60)}h {hike.duration % 60}m</Text>
        </View>
      </View>
    ...
    <LinearGradient colors={T.bgGrad} style={{ flex: 1 }}>
      ...
      >
        <Animated.View entering={FadeInDown.delay(60).duration(600)} style={p.header}>
          <View style={{ flex: 1, gap: 2 }}>
            <Text style={p.eyebrow}>NEARBY HIKES</Text>
            <Text style={p.title}>Hikes Near Me</Text>
          </View>
        ...
        <Animated.View entering={FadeInDown.delay(120).duration(600)}>
          <View style={p.notice}>
            ...
            <Text style={p.noticeText}>
              Showing popular UK trail suggestions.{" "}
              {/* TODO: Connect to location API for personalised nearby hikes */}
        ...
        <Animated.View entering={FadeInDown.delay(180).duration(600)} style={{ gap: 12 }}>
          {MOCK_HIKES.map((hike, i) => (
            <HikeCard key={i} hike={hike} onLog={name => openLog(name)} />
          ))}
        </Animated.View>
...
const p = StyleSheet.create({
  ...
  notice: {
    ...
    backgroundColor: T.surface, borderRadius: 10, padding: 10, borderWidth: 1, borderColor: T.border,
  },
  noticeText: { flex: 1, fontSize: 11, fontFamily: "Inter_400Regular", color: T.textDim, lineHeight: 16 },
