...
describe("both screens state the same readiness", () => {
  ...
  it("colors the displayed V2 score, not the legacy context score", () => {
    const plan = code(read("app/(tabs)/plan.tsx"));
    expect(plan).toMatch(/readinessColor = heroReadiness === null/);
    expect(plan).not.toMatch(/readinessColor = readinessScore/);
  });
});

describe("a plan week is shown in its own dates", () => {
  it("swipes through the same order as the displayed week strip", () => {
    const plan = code(read("app/(tabs)/plan.tsx"));
    expect(plan).toMatch(/displayOrder = viewedWeekDays\.map\(day => day\.dow\)/);
    expect(plan).toMatch(/currentDowIdx = displayOrder\.indexOf\(selectedDow\)/);
    expect(plan).toMatch(/setSelectedDow\(planWeekDays\(nextW\.startDate\)\[0\]\.dow\)/);
    expect(plan).not.toMatch(/DISPLAY_ORDER/);
  });
  ...
  it("keeps seven consecutive calendar dates across a daylight-saving change", () => {
    const days = planWeekDays("2026-03-27", new Date("2026-04-15T09:00:00"));
    expect(days.map(d => d.date)).toEqual([27, 28, 29, 30, 31, 1, 2]);
    expect(days.map(d => d.dow)).toEqual([5, 6, 0, 1, 2, 3, 4]);
  });
